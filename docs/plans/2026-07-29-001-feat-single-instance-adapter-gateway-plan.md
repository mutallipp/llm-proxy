...
- **权威配置**：复用 `ModelSettings.Associations`；每个协议池由池 key 表达入站/出站协议，池内 association 保存 Channel、physical model、priority、enabled 及既有匹配条件。
...
4. 协议池决定出站协议，池内 target 不填写 `outboundApiFormat`。配置池时，Channel 下拉仅展示支持该协议 Endpoint 的渠道（依据渠道 `endpoints`/`defaultEndpoints` 能力）。
5. Model 内继续复用 `ModelSettings.Associations` 与已有 `ModelAssociation` matcher，不创建 Association 表或平行 Model 实体。
6. ModelGroup、ModelGroupProtocol、ModelGroupTarget 不再是领域、API、UI 或运行时概念；旧表仅作一次性迁移来源，验证后物理删除，不双写、不 fallback。
...
- **KTD-006（session-settled，替换原建议）**：复用 `ModelSettings.Associations` JSON 与既有 matcher；增加协议池标识/结构，不引入独立 Association 表或平行 Model。
...
- **文件**：`internal/objects/model.go`；`internal/ent/schema/model.go`；`internal/server/biz/model_association_matcher.go`；相关对象/JSON 测试文件。
...
- **验证**：对象序列化/反序列化、matcher 单测覆盖协议隔离和 priority；schema/API 校验拒绝非法池。

### IU-02：ModelGroup 一次性回填、冲突报告与回滚

- **文件**：`internal/ent/schema/model_group*.go`、相关迁移文件；`internal/server/biz` 迁移服务；迁移/回滚测试文件。
...
- **实现要点**：按 logical model 确定性合并协议池；inbound/outbound 不一致、缺 Model、重复映射、不可表达记录逐条报告并阻止；事务提交前预览，失败恢复，不双写。
- **场景**：happy：`gpt-5.6-luna` 正确回填 openai/responses target；edge：同池多个 target、重复 priority、已有 settings 合并；error：协议冲突、缺目标、绑定歧义；integration：预览→事务→读回→回滚。
- **验证**：固定 fixture 断言目标 JSON、报告内容、事务原子性；确认迁移后旧数据未被静默跨协议改写。

### IU-03：Adapter binding 改为 model_id

- **文件**：`internal/ent/schema/adapter_model_binding.go`；`internal/server/biz/adapter.go`；`internal/server/api/gateway.go`；Adapter DTO/GraphQL schema/resolver 及对应测试。
- **现有模式**：沿用现有 Adapter binding REST/DTO、DEFAULT 绑定语义和 Ent 外键迁移方式。
- **实现要点**：将 `model_group_id` 替换为 `model_id`，更新唯一约束、输入输出、查询预加载、删除校验；一个 Model 可被不同 inbound protocol Adapter 复用，Adapter 不暴露 Channel 字段。
- **场景**：happy：两个协议 Adapter 绑定同一 Model；edge：重复 DEFAULT、未知 Model、不同协议合法共存；error：提交 ModelGroup ID 或 Channel 直连字段；integration：迁移后的 `pi-openai-v2` binding 读回正确。
- **验证**：schema 迁移检查、API contract tests、唯一约束和错误响应测试。
...
- **实现要点**：snapshot 从 binding 取 Model，使用 Adapter 固定 inbound protocol 选池；仅在该池选择 Channel/physical model，保留 enabled、priority、failover；移除 ModelGroup fallback 和跨协议转换。
- **场景**：happy：openai Adapter 只走 openai pool；edge：最高 priority 不健康转下一个、池只有一个 target；error：无 binding、无池、全禁用、协议不匹配；integration：真实请求验证协议转换与响应恢复链路。
- **验证**：selector 单测、快照刷新测试、真实 Adapter 请求及失败转移证据；确认日志不出现 ModelGroup 查询。

### IU-05：`/models` 协议池配置与共享 Model UI

- **文件**：`frontend/src/routes`、`frontend/src/features`、`frontend/src/gql` 中 `/models` 相关文件；后端 Model API/GraphQL 文件；前端测试。
...
- **场景**：happy：新增 openai/responses 池并选择 `cider-openai`；edge：渠道能力变化、空池、禁用 target、同 Model 多绑定；error：前端提交不支持 endpoint 或跨协议字段；integration：浏览器创建、保存、刷新后配置一致。
- **验证**：类型/组件测试、API contract、浏览器表单与过滤验收；确认旧 ModelGroup UI/routes 不可达。

### IU-06：ModelGroup 概念与旧表下线

- **文件**：`internal/server/api/gateway.go`、`internal/server/biz/adapter.go` 及 ModelGroup API/UI/routes/schema/运行时引用；Ent 旧 schema/迁移文件。
...
- **验证**：源码依赖扫描、schema migration review、API 404/不注册验证、迁移前置条件测试。

### IU-07：全链路验收与回滚证明

- **文件**：后端 API/运行时测试、前端浏览器测试、迁移验证 fixture 和回滚测试所在仓库相对路径。
- **现有模式**：沿用项目既有后端测试、前端浏览器验收、Adapter 集成测试和快照验证方式。
- **实现要点**：覆盖协议隔离、共享 Model、优先级故障转移、endpoint 过滤、迁移报告、旧表删除前后回滚；不将无关功能纳入验收。
- **场景**：happy：openai 与 anthropic Adapter 分别命中各自池；edge：同 Model 多 Adapter、目标健康变化；error：协议冲突、空池、回滚触发；integration：真实 Adapter 请求返回预期上游协议响应。
- **验证**：后端/前端/浏览器/真实 Adapter 链路证据齐全；迁移失败可恢复；记录删除旧表前的验证结论。
