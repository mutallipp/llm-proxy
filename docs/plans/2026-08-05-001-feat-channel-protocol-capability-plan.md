...
- U2. **ent schema 与迁移**。Modify: `internal/ent/schema/channel.go`（`field.JSON("protocol_capabilities", objects.ChannelProtocolCapabilities{}).Default(...).Optional().Annotations(entgql.Skip(entgql.SkipMutationCreateInput, entgql.SkipMutationUpdateInput))`——Skip 防止字段进入 Create/UpdateChannelInput 形成绕过写入面，对齐 KTD18 唯一保存面）。Run: `make generate`；enttest 验证字段读写与自动迁移。Depends: U1。
