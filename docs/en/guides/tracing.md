...
### Overview
llm-proxy captures every inbound request in a thread-aware trace without forcing you to adopt a new SDK. If your client already speaks the OpenAI-compatible protocol, you can opt into observability simply by forwarding trace and thread headers—or rely on llm-proxy to create them for you automatically.
...
- **Performance Optimization**: llm-proxy prioritizes routing requests within the same Trace to the same upstream channel. This significantly improves provider-side cache hit rates (e.g., Anthropic's Prompt Caching), reducing latency and lowering costs.
...
- **Trace ID (`AH-Trace-Id`)** – Represents a single user message and all the agent requests it triggers. You must provide this header when you need multiple requests to be linked; omitting it causes llm-proxy to record requests separately even though it can auto-generate IDs.
...
- Provide `AH-Trace-Id` when you need sequential requests to appear under the same trace. Without it llm-proxy will log them independently, even though autogenerated IDs are available for standalone calls.
...
- Configure a default storage location in the admin console; llm-proxy will fall back to the primary storage if the preferred option is unavailable.
...
- Turn on Claude Code extraction with `server.trace.claude_code_trace_enabled: true` so llm-proxy can pick up trace IDs automatically.
...
- If you already send a trace header, llm-proxy keeps your value—manual instrumentation and auto-extraction work together.

### Codex Trace Support
- Turn on Codex extraction with `server.trace.codex_trace_enabled: true` so llm-proxy can reuse the `Session_id` header as the trace ID.
- If you already send a trace header, llm-proxy keeps your value—manual instrumentation and auto-extraction work together.

### Exploring Traces in the Console
1. Navigate to **Traces** in the llm-proxy admin console.
...
While llm-proxy theoretically supports unlimited requests per trace, we recommend the following limits in production:
