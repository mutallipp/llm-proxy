...
本文介绍一个请求进入 llm-proxy 后，大致会经过哪些步骤。它的目标是帮助你建立一个简单、准确的心智模型。

## 一句话总结

llm-proxy 会先把请求转换成统一格式，再根据配置选择渠道，最后把结果转换回客户端期望的格式。

## 核心概念：三层模型设置

llm-proxy 中有三个地方会影响模型名称：
...
llm-proxy 支持多种 API 格式（如 OpenAI、Anthropic、Gemini），会先把请求转换成统一的内部格式。
