...
llm-proxy 实现了一个复杂的双向数据转换管道，确保客户端和 AI 提供商之间的无缝通信。本文档解释了系统中的架构组件和数据流。
...
**组件：**
- **Web 应用程序**：使用 llm-proxy API 的基于浏览器的应用程序
- **移动应用程序**：使用 llm-proxy 服务的原生移动应用
...
1. **流式设置**
   ```
   客户端 → 流式请求 → llm-proxy → 提供商流
   ```

2. **数据流**
   ```
   提供商分块 → llm-proxy 处理 → 客户端分块
   ```

3. **流式完成**
   ```
   提供商结束 → llm-proxy 清理 → 客户端完成
   ```
