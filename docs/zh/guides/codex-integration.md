...
## 概览
llm-proxy 可以作为 OpenAI 接口的直接替代方案，使 Codex 能够通过您自己的基础设施连接。本文将介绍配置方法，并说明如何结合 llm-proxy 的模型配置文件功能实现灵活路由。

### 关键点
- llm-proxy 支持多种 AI 协议/格式转换。你可以配置多个上游渠道（provider/channel），对外提供统一的 OpenAI 兼容接口，供 Codex 使用。
...
### 配置 Codex
1. 编辑 `${HOME}/.codex/config.toml`，将 llm-proxy 注册为 provider：
   ```toml
   model = "gpt-5"
...
#### 按对话聚合 Trace（重要）
开启内置 Codex 追踪提取后，llm-proxy 会将 `Session_id` header 作为 trace ID 使用：
...
#### 验证
- 发送测试 Prompt，llm-proxy 日志中应出现 `/v1/chat/completions` 调用。
- 启用 llm-proxy 的追踪功能可查看提示词、回复及延迟信息。
...
### 常见问题
- **Codex 认证失败**：确保在启动 Codex 的同一 shell 会话中设置了 `LLM_PROXY_API_KEY`。
